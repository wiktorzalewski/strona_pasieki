...
// JSON-LD: Recipe schema dla każdego przepisu
$recipeSchemas = [];
foreach ($recipes as $recipe) {
    $ingredients = json_decode($recipe['ingredients'], true) ?: [];
    $steps = json_decode($recipe['steps'], true) ?: [];
    ...
    $recipeSchemas[] = [
        '@type' => 'Recipe',
        'name' => $recipe['title'],
        'image' => 'https://pasiekapodgruszka.pl/' . $recipe['image_path'],
        'author' => [
            '@type' => 'Person',
            'name' => 'Ciocia Agnieszka'
        ],
        'prepTime' => 'PT' . preg_replace('/[^0-9]/', '', $recipe['prep_time']) . 'M',
        'description' => $recipe['short_desc'],
        'recipeIngredient' => $ingredients,
        'recipeInstructions' => $instructionSteps,
        'recipeCategory' => 'Przepisy z miodem',
...
$extraJsonLd = '';
foreach ($recipeSchemas as $schema) {
    $schema['@context'] = 'https://schema.org';
    $extraJsonLd .= '<script type="application/ld+json">' . json_encode($schema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) . '</script>';
}
